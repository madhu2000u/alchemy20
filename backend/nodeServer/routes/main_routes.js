...
main_router.post(mainRoutes.events_route, utils.validatePostapi, (req, res) => {
	...
	if (
		...
		//Object.keys(body.event).length===0 is not user cuz when particular attribute such as event_desc is not event send in the body, it gets created.
...
main_router.put(mainRoutes.events_route, utils.validatePostapi, (req, res) => {
	//HTTP put is for update, and utils.validatePostapi because it for use for the admin prople, so wheter it is a post or a put request doesnt matter the api_key if it is validatePostapi for put instead of a separate utils.validatePutapi
