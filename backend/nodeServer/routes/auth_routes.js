...
router.get('/auth', (req, res) => {
	...
	user.findOne({email: body.email}, (err, result) => {
		...
		if (err) {
			...
			user_tokens.findOne({user_id: result._id}, (err, user_tokens_result) => {
...
router.post('/auth', (req, res) => {
	...
	if (
		...
		res.status(400);
		res.json({message: 'Bad request. One or more requried fields are missing'});
...
router.get('/confirm/:verification_token', (req, res) => {
	temp_actvn_user
		.findOne({verification_token: req.params.verification_token})
		.then((actvn_result) => {
			...
			if (actvn_result != null) {
				user.updateOne({_id: actvn_result.user_id}, {acc_active: true}).then((result) => {
					...
					temp_actvn_user.deleteOne({user_id: actvn_result.user_id}, (err) => {
...
router.post('/verify_mail_resend', (req, res) => {
	// email shoud be sent in header (let it be as it is)
