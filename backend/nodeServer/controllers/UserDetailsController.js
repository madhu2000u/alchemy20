...
exports.AddUserDetails = (req, res) => {
    ...
    if(!body.name || !body.mobile || !body.college || !body.dept || !body.year_of_study){res.status(401).json({message:"Required fields missing"})}
    ...
    User.findOne({_id:req.user.id}).then((result)=>{
        if(!result){
            ...
            User.updateOne({_id:req.user.id}, body).then((result) =>{
