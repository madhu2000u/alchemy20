...
exports.updateSponsor = (req, res) => {
	if (!req.headers['sponsor_id']) {
		...
		return res.status(403).json({message: 'Update data missing in body'});
	}
	sponsors.find({}).then((result) => {
		if (result[0].sponsors.length == 0) {
			return res.status(404).json({message: 'No sponsors found to update them'});
		}

		sponsors
			.updateOne(
				{_id: result[0]._id, 'sponsors._id': req.headers['sponsor_id']},
				{
					$set: {
						'sponsors.$.portfolio': req.body.portfolio,
						'sponsors.$.img_url': req.body.img_url,
						'sponsors.$.websiteUrl': req.body.websiteUrl,
					},
				}
			) //https://docs.mongodb.com/manual/reference/operator/update/positional/ (To update embedded objects in a list inside a document)
			.then((result) => {
				console.log('update sponsors - ', result);
				if (result.n == 0) {
					return res.status(404).json({message: 'Specified sponsor not found'});
				}
...
exports.deleteSponsor = (req, res) => {
	...
	sponsors.findOne({}).then((result) => {
		if (result[0].sponsors.length == 0) {
			return res.status(403).json({message: 'No sponsors found to delete them'});
		}

		sponsors.update({_id: result[0]._id}, {$pull: {sponsors: {_id: req.headers['sponsor_id']}}}).then((result) => {
